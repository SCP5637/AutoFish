...
3. 在阶段 4 ExitPlanMode 批准后直接写入 `AUTOFISH_PROJECT_DOC`。
4. 在阶段 5 按用户明确确认修改 `AUTOFISH_PROJECT_CONFIG`。
5. bootstrap 完成后停止，不再继续业务开发。

你禁止做以下事情：
- 不得自行推测产品方向。
- 无需额外打字确认，ExitPlanMode 批准即视为确认。
- 不得修改业务代码。
- 不得提交 git。
- 不得安装依赖。
- 不得生成额外文档。
...
阶段 1 结束时，你必须使用多次换行隔断开上文，然后用颜色醒目的字体明确的对用户说明以下文段(注意第一句话"好的，现在我已经......"使用高亮有颜色的字体，以及不要在以下文段中添加额外的内容，这一段文段是作为引导用户继续问答而使用)：

> 好的，现在我已经理解清楚了 AutoFish 规范和项目基本情况，现在请提出你的具体想法。
>
> 我需要你尽可能有条理、详细地描述你的需求和想法，作为 AutoFish 后续自动开发与分析的指引。信息越充分，后续自动执行就越不容易卡在"设计决策不明确"上。
>
> 建议以以下方向思考和组织你的描述（可以只写你知道的部分，不确定的可以说"待定"）：
>
> **1. 项目目标**
> - 这个项目最终要做什么？解决什么问题？给谁用？
> - 有没有参考项目或竞品？它们的什么特点是你想要的/不想要的？
>
> **2. 功能清单**
> - 按优先级列出所有功能模块，能拆多细就多细
> - 每个功能：输入是什么 → 输出是什么 → 用户怎么操作
> - 哪些功能希望是必须实现的？哪些是后续再考虑的？
>
> 如果你暂时只有模糊的想法也没关系，把你知道的写出来，不确定的地方我会在后续追问中帮你细化。

### 阶段 2：多轮细化需求 / 模块 / 效果（plan mode）

进入这一阶段前，你必须先调用 `EnterPlanMode`。

在这一阶段：
- 继续追问目标、范围、模块、效果、输入输出、验收标准。
- 优先用 `AskUserQuestion` 收集多选/单选决策，减少零散对话。
- 遇到多个合理方案时，列出选项，让用户选。
- 不能写 `project.md`。
- 只能沉淀“准备写入 project.md 的结构草案”。

### 阶段 3：询问特殊开发偏好（plan mode）
...
### 阶段 4：先展示 project.md 草案，ExitPlanMode 批准后直接写入（plan mode → 普通会话）

你必须分两步做：
1. 在 plan mode 中整理准备写入的 `project.md` 结构草案，至少包括：
   - `## 技术栈`
   - `## 开发规则`
   - `## 任务清单`
   - 分阶段任务、验收标准、`[BLOCK]`、`[RESET]`
2. 草案准备好后，把草案写入 plan file，调用 `ExitPlanMode` 向用户展示草案并请求批准。批准后直接写入（无需额外打字确认）。
...
### 阶段 5：询问是否需要定制 AutoFish config.json（普通会话）
...
- 如果用户说需要：
  - 继续问答收集配置需求
  - 先展示配置草案
  - ExitPlanMode 批准后直接修改 `AUTOFISH_PROJECT_CONFIG`
  - 然后更新：

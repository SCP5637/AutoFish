...
### 阶段 4：先展示 project.md 草案，再等待明确确认后写入（plan mode → 普通会话）
...
2. 草案准备好后，把草案写入 plan file，调用 `ExitPlanMode` 向用户展示草案并请求批准。
3. 用户批准退出 plan mode 后，回到普通会话，明确要求用户回复“确认生成 project.md”；只有收到这句后，才允许写入 `AUTOFISH_PROJECT_DOC`。
